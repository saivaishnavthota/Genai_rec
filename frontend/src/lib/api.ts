...
export const aiInterviewAPI = {
  ...
  getQuestionAudio: async (sessionId: number, questionId: number): Promise<Blob> => {
    const response = await apiClient.get(`/api/ai-interview/${sessionId}/questions/${questionId}/audio`, {
      responseType: 'blob'
    });
    return response.data;
  },
